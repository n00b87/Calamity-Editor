...
MAX_IMAGECLIPS = 200
MAX_IMAGESLIDES = 200
MAX_IMAGESLIDE_CLIPS = 200
MAX_IMAGES = 1024
MAX_SURFACES = 50
MAX_BUTTONTOGGLEGROUPS = 50
...
Dim Gui_Button_ToggleFlag[MAX_BUTTONS]
Dim Gui_Button_Toggle_Value[MAX_BUTTONS]
Dim Gui_Button_Toggle_Group[MAX_BUTTONS]

Dim Gui_ButtonToggleGroup_Buttons[MAX_BUTTONTOGGLEGROUPS, MAX_BUTTONS]
Dim Gui_ButtonToggleGroup_Exists[MAX_BUTTONTOGGLEGROUPS]
Dim Gui_ButtonToggleGroup_ButtonCount[MAX_BUTTONTOGGLEGROUPS]
...
Sub Gui_SetWindowActive(win_id)
	...
	MOUSE_STATE = 0
	mb_right = 0
	mb_doubleClick_timer = 0
	MOUSE_DBLCLICK_EVENT = 0
	MOUSE_DBLCLICK_VALUE = 0
End Sub
...
Sub Gui_TabRefresh(id)
	...
	old_AT = new_AT
	new_AT = Gui_TabGroup_ActiveTab[index]
...
Function Gui_Button_GetValue(button_id)
	button_index = Gui_Element_Index[button_id]
	Return Gui_Button_Toggle_Value[button_index]
End Function

Sub Gui_Button_SetValue(button_id, b_value)
	button_index = Gui_Element_Index[button_id]
	Gui_Button_Toggle_Value[button_index] = (b_value <> 0)
	Gui_Button_Current_Image[button_index] = Gui_Button_Toggle_Value[button_index]
End Sub

Sub Gui_Button_SetToggleFlag(button_id, toggle_flag)
	button_index = Gui_Element_Index[button_id]
	Gui_Button_ToggleFlag[button_index] = toggle_flag
	If Not toggle_flag Then
		Gui_Button_Current_Image[button_index] = 0
	End If
End Sub

Function Gui_Button_GetToggleFlag(button_id)
	button_index = Gui_Element_Index[button_id]
	Return Gui_Button_ToggleFlag[button_index]
End Function

Function Gui_CreateButtonToggleGroup()
	For i = 0 to MAX_BUTTONTOGGLEGROUPS-1
		If Not Gui_ButtonToggleGroup_Exists[i] Then
			Gui_ButtonToggleGroup_Exists[i] = True
			Return i
		End If
	Next
	Return -1
...
Sub Gui_ClearButtonToggleGroup(t_group)
	If t_group < 0 Then 
		Return
	End If
	Gui_ButtonToggleGroup_ButtonCount[t_group] = 0
	Gui_ButtonToggleGroup_Exists[t_group] = False
End Sub

Sub Gui_ButtonToggleGroup_AddButton(t_group, button_id)
	If t_group < 0 Or button_id < 0 Then
		Return
	End If
	...
	button_index = Gui_Element_Index[button_id]
	b_count = Gui_ButtonToggleGroup_ButtonCount[t_group]
	Gui_ButtonToggleGroup_Buttons[t_group, b_count] = button_id
	Gui_Button_Toggle_Group[button_index] = t_group
	Gui_ButtonToggleGroup_ButtonCount[t_group] = b_count + 1
End Sub

Sub Gui_ButtonToggleGroup_RemoveButton(t_group, button_id)
	If t_group < 0 Or button_id < 0 Then
		Return
	End If
	
	flag = false
	For i = 0 to Gui_ButtonToggleGroup_ButtonCount[t_group]-1
		If Gui_ButtonToggleGroup_Buttons[t_group, i] = button_id Then
			flag = true
		End If
		
		If flag Then
			Gui_ButtonToggleGroup_Buttons[t_group, i] = Gui_ButtonToggleGroup_Buttons[t_group, i+1]
		End If
	Next
	
	Gui_ButtonToggleGroup_ButtonCount[t_group] = Gui_ButtonToggleGroup_ButtonCount[t_group] - 1
	
End Sub

Function Gui_ButtonToggleGroup_GetActiveButton(t_group)
	For i = 0 to Gui_ButtonToggleGroup_ButtonCount[t_group]-1
		button_id = Gui_ButtonToggleGroup_Buttons[t_group, i]
		button_index = Gui_Element_Index[button_id]
		If Gui_Button_Toggle_Value[button_index] Then
			Return button_id
		End If
	Next
	Return -1
...
Function Gui_CreateButton(txt$, w, h)
	...
	For i = 0 to MAX_BUTTONS-1
		If Not Gui_Button_Exists[i] Then
			...
			Gui_Button_ToggleFlag[i] = False
			Gui_Button_Toggle_Value[i] = 0
			Gui_Button_Toggle_Group[i] = -1
			Gui_Button_W[i] = w
			Gui_Button_H[i] = h
...
Function Gui_CreateImageButton(img_file$, w, h)
	If w < 8 Or h < 8 Then
		Return -1
	End If
	...
	dim img_w
	dim img_h
	b_img = Gui_GetFreeImage()
	LoadImage(b_img, img_file$)
	
	If Not ImageExists(b_img) Then
		Return -1
	End If
	
	GetImageSize(b_img, img_w, img_h)
	...
	For i = 0 to MAX_BUTTONS-1
		If Not Gui_Button_Exists[i] Then
			...
			SetColor(GUI_BUTTON_COLOR1)
			RectFill(0, 0, w, h)
			DrawImage_Blit_Ex(b_img, 2, 2, w-4, h-4, 0, 0, img_w, img_h) 
			CanvasClip(img, 0, 0, w, h, 1)
...
Function Gui_ListBox_GetCount(lbox)
	lbox_index = Gui_Element_Index[lbox]
	Return Gui_ListBox_Count[lbox_index]
End Function
...
Function GetDoubleClick(mb)
		t = (timer - mb_doubleClick_timer)
		If mb Then
			Select Case MOUSE_DBLCLICK_EVENT
			Case 0
				MOUSE_DBLCLICK_EVENT = 1
			Case 2
				If t < 220 Then
					MOUSE_DBLCLICK_EVENT = 0
					MOUSE_DBLCLICK_VALUE = 1
					'Print "D: ";t
					Return True
				Else
					MOUSE_DBLCLICK_EVENT = 0
				End If
			End Select
		Elseif MOUSE_DBLCLICK_EVENT = 1 Then
			MOUSE_DBLCLICK_EVENT = 2
			mb_doubleClick_timer = timer
		ElseIf MOUSE_DBLCLICK_EVENT = 2 And t >= 220 Then
			MOUSE_DBLCLICK_EVENT = 0
		End If
...
Function Gui_DoubleClicked()
	Return MOUSE_DBLCLICK_VALUE
End Function
	...
	Else
		For i = 0 to MAX_GUI_ELEMENTS-1
			If Gui_Element_Active[i] And Gui_Element_Active[ Gui_Element_Parent[i] ] And Gui_Element_Window[ Gui_Element_Parent[i] ] = gui_current_win Then
				...
				Case GUI_TYPE_BUTTON
					If Gui_Button_ToggleFlag[obj_index] Then
						Gui_Button_Current_Image[obj_index] = Gui_Button_Toggle_Value[obj_index]
						...
						ElseIf EVENT_TYPE = 0 And IsOnObject(i,mx,my) And MOUSE_STATE = MOUSE_PRESSED Then
							...
							EVENT_ID = i
							Gui_Button_Toggle_Value[obj_index] = Not Gui_Button_Toggle_Value[obj_index]
							Gui_Button_Current_Image[obj_index] = Gui_Button_Toggle_Value[obj_index]
							
							'print "DBG: "; Gui_Button_Toggle_Group[obj_index]; ", "; Gui_Button_Toggle_Value[obj_index]
							If Gui_Button_Toggle_Group[obj_index] >= 0 And Gui_Button_Toggle_Value[obj_index] Then
								'print "test"
								t_group = Gui_Button_Toggle_Group[obj_index]
								For t_group_button = 0 To Gui_ButtonToggleGroup_ButtonCount[t_group]-1
									If Gui_ButtonToggleGroup_Buttons[t_group, t_group_button] <> i Then
										Gui_Button_SetValue(Gui_ButtonToggleGroup_Buttons[t_group, t_group_button], 0)
									End If
								Next
							End If
				...
				Case GUI_TYPE_IMAGESLIDE
					...
					ElseIf EVENT_TYPE = 0 And IsOnObject(i,mx,my) And MOUSE_STATE = MOUSE_PRESSED Then
						...
						'Print "is: "; Gui_ImageSlide_SelectedClip[obj_index]
						Return
					ElseIf EVENT_TYPE = 0 And IsOnObject(i,mx,my) And MOUSE_STATE = MOUSE_HOLD Then
						'mouse is down before going over button
						Return
...
Function Gui_AppendDir$(base_dir$, append_dir$)
	delimeter$ = "/"
	...
	Return base_dir$ + delimeter$ + append_dir$
End Function

Function Gui_GetDirSeparator$()
	delimeter$ = "/"
